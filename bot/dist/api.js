...
export async function getPublicTariffs(token) {
    if (token)
        return fetchJson("/api/client/tariffs", { token });
...
export async function postBotAdminBroadcast(telegramId, message, channel, photoFileId) {
    const botToken = process.env.BOT_TOKEN || "";
