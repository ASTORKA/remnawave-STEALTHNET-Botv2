                ...
                <CollapsibleContent className="pt-3 space-y-3">
                  ...
                  <div className="rounded-md border overflow-x-auto overflow-hidden">
                    <table className="w-full text-sm min-w-[400px] [&_th]:whitespace-nowrap [&_td]:whitespace-nowrap">
                      <thead>
                        <tr className="border-b bg-muted/50">
                          <th className="text-left p-2 font-medium w-40">TG emoji ID</th>
                          <th className="text-left p-2 font-medium">Название</th>
                      ...
                      <tbody>
                        {(settings.sellOptionsDevicesProducts ?? []).map((p, i) => (
                          <tr key={p.id} className="border-b last:border-0">
                            <td className="p-2"><Input className="h-9 w-full min-w-[160px] text-xs" placeholder="6037286673010660132" value={p.tgEmojiId ?? ""} onChange={(e) => setSettings((s) => { if (!s?.sellOptionsDevicesProducts) return s; const arr = [...s.sellOptionsDevicesProducts]; arr[i] = { ...arr[i], tgEmojiId: e.target.value }; return { ...s, sellOptionsDevicesProducts: arr }; })} /></td>
                  ...
                  <div className="mt-3">
                    <Button type="button" variant="outline" size="sm" onClick={() => setSettings((s) => (s ? { ...s, sellOptionsDevicesProducts: [...(s.sellOptionsDevicesProducts ?? []), { id: `devices_${Date.now()}`, name: "", deviceCount: 1, price: 0, currency: "rub", tgEmojiId: "" }] } : s))}>
                      <Plus className="h-4 w-4 mr-1" /> Добавить
                    </Button>
                  </div>
                </CollapsibleContent>
