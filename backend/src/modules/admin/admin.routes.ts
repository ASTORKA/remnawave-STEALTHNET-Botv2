...
const updateSettingsSchema = z.object({
  ...
  botBackEmojiKey: z.string().max(40).nullable().optional(),
...
adminRouter.patch("/settings", async (req, res) => {
  ...
  if (updates.botBackEmojiKey !== undefined) {
    const val = (updates.botBackEmojiKey ?? "").trim();
    await prisma.systemSetting.upsert({
      where: { key: "bot_back_emoji_key" },
      create: { key: "bot_back_emoji_key", value: val },
      update: { value: val },
    });
  }
...
const autoBroadcastRuleSchema = z.object({
  ...
  instantOnExpire: z.boolean().optional(),
  channel: z.enum(["telegram", "email", "both"]),
...
adminRouter.get("/auto-broadcast/rules", asyncRoute(async (_req, res) => {
  ...
  return res.json(
    rules.map((r) => ({
      ...
      instantOnExpire: (r as Record<string, unknown>).instantOnExpire === true,
      channel: r.channel,
      subject: r.subject,
      message: r.message,
...
adminRouter.post("/auto-broadcast/rules", asyncRoute(async (req, res) => {
  ...
  const rule = await prisma.autoBroadcastRule.create({
    data: {
      ...
      instantOnExpire: data.instantOnExpire ?? false,
  ...
  return res.status(201).json({
    ...
    instantOnExpire: (rule as Record<string, unknown>).instantOnExpire === true,
